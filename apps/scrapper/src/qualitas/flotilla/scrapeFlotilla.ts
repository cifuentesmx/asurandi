import { ScrappedFlota, ScrappedFlotaInciso } from '@asurandi/types';
import { scrapeSiniestros } from '../poliza/scrapeSiniestros.js';
export async function scrapeFlotilla(numero_poliza: string, browser: WebdriverIO.Browser): Promise<ScrappedFlota> {


    await browser.url(`https://agentes360.qualitas.com.mx/group/guest/consulta-de-flotas/-/detalle/poliza?numPoliza=${numero_poliza}`)
    const flotilla: ScrappedFlota = {
        resumen: [],
        incisos: []
    }
    ...
    for (let i = 0; i < lis.length; i++) {
        ...
        if (!scrappedIncisos.includes(id)) {
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                const data: ScrappedFlotaInciso = {
