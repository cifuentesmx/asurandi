...
if (dev && PUBLIC_FIREBASE_CONNECT_EMULATORS === 'true') {
    try {
        connectAuthEmulator(auth, `http://${PUBLIC_FIREBASE_EMULATORS_HOST}:9099`, { disableWarnings: true })
        connectFirestoreEmulator(db, `${PUBLIC_FIREBASE_EMULATORS_HOST}`, 8080)
        connectStorageEmulator(storage, `${PUBLIC_FIREBASE_EMULATORS_HOST}`, 9199)
        connectFunctionsEmulator(functions, `${PUBLIC_FIREBASE_EMULATORS_HOST}`, 5001)
    } catch (error) {
        console.error(error)
        console.error('// TODO unified handle error')
    }
