<script lang="ts">
	...
	let { accounts = $bindable(), account } = $props<{
...
<Sheet.Root bind:open>
	<Sheet.Trigger>
		<Button variant="outline">{account.alias} - {account.cuenta}</Button>
	</Sheet.Trigger>
	<Sheet.Content side="right">
		<ScrollArea class="h-full w-full p-4">
			...
			>
				<Sheet.Header>
					<Sheet.Title>Editar cuenta</Sheet.Title>
					<Sheet.Description>
				...
				<div class=" py-4">
					<DeleteAccount bind:accounts {account} />
				</div>
				<Sheet.Footer>
					<Sheet.Close>
